...
function ValidateSyntax(_rawCode) {
    ...
    for (var i = 0; i < array_length(lines); i += 1) {
        ...
        if (string_starts_with(line, "function")) {
            ...
            var closeParenPos = string_pos(")", line); // Use first ')' for function definition itself
        ...
        if (line == "{") 
        {
            ...
            // Ensure the block expecting this brace hasn't already been marked as opened
        ...
        if (line == "}") 
        {
            ...
            if (!variable_struct_exists(currentBlock, "hasOpened") || !currentBlock.hasOpened) 
            {
                ...
                return { 
                    Valid: false, 
                    Error: string_concat("Line ", string(lineNumber), ": Closing brace '}}' found, but expected block (e.g., Repeat or function on line ", string(currentBlock.lineNumber), ") seems incomplete (missing '{{').") 
                };
            }
        ...
        if (string_starts_with(line, "Repeat(")) 
        {
            ...
            else 
            {
                ...
                { 
                    ...
                    if (innerName != "" && global.vars.CommandLibrary[$ string_lower(innerName)] != undefined) { isValidParam = true; }
                    else 
                    { 
                        ds_list_destroy(declaredVariables); 
                ...
                else if (string_lettersdigits(repeatParam) == repeatParam && repeatParam != "") { // Variable
        ...
        if (string_starts_with(line, "var ")) 
        {
            ...
            if (array_length(parts) == 2) 
            {
                ...
                if (string_starts_with(declarationPart, "var ")) 
                {
                    ...
                    { 
                        ds_list_destroy(declaredVariables); 
                        return { Valid: false, Error: string_concat("Line ", string(lineNumber), ": Variable '", varName, "' needs numeric value. Found '", valuePart, "'.") }; 
                    }
        ...
        if (openParen > 0) {
            ...
            if (closeParen > openParen) {
                ...
                param = string_trim(string_copy(line, openParen + 1, closeParen - openParen - 1)); // Extract param between first '(' and last ')'
                ...
                // Validate parameter (logic remains similar)
                if (param != "") 
                {
            ...
            { 
                ...
                ds_list_destroy(declaredVariables);
                return { Valid: false, Error: string_concat("Line ", string(lineNumber), ": Mismatched or missing closing parenthesis for command '", string_copy(line, 1, openParen-1) ,"'.") }; // Use string_concat
            }
        } 
        else 
    ...
    { 
        ds_list_destroy(declaredVariables); 
        return { Valid: false, Error: "Syntax Error: Unclosed block (likely Repeat)." }; 
    }
