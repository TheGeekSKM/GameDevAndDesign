...
function StartInterpreter(_compiledStruct) {
    if (_compiledStruct == undefined || (variable_struct_exists(_compiledStruct, "Errors") && _compiledStruct.Errors != undefined)) {
        ...
        if (variable_struct_exists(_compiledStruct, "Errors") 
            && _compiledStruct.Errors != undefined 
            && array_length(_compiledStruct.Errors) > 0) 
        { 
            errorMsg = string_concat(errorMsg, "\nFirst Error: ", _compiledStruct.Errors[0]); 
        } 
        
        show_message(errorMsg); isRunning = false; return;
    }
...
function ExecuteInstruction(_instr) 
{
    if (_instr == "" || _instr == undefined) return;
    ...
    if (string_starts_with(_instr, "DeclareVar(")) 
    {
        ...
        { 
            show_debug_message(string_concat("Error: Malformed DeclareVar: ", _instr)); StopInterpreter(); 
        } 
        return;
    }
    ...
    if (string_starts_with(_instr, "RepeatStart(")) 
    {
        ...
        if (commaPos > 0 && jumpOffsetPos > commaPos) 
        {
            ...
            if (colonPos > 0) 
            {
                ...
                if (paramType == "number") loopCount = real(paramValue);
                else if (paramType == "lookup") 
                {
                    ...
                    {   show_debug_message(string_concat("Error: Variable '", paramValue, "' not found for Repeat.")); 
                        StopInterpreter(); 
                        return; 
                    } 
                } 
                else if (paramType == "result") 
                {
                    ...
                    {
                        show_debug_message("Warning: Repeat using [result] but last result undefined. Repeating 0 times.");
                        loopCount = 0;
                    }
                } 
                else 
            ...
            if (loopCount <= 0) 
            {
                ...
                show_debug_message(string_concat("Repeat count is ", string(loopCount), ", skipping loop block."));
            } 
            else 
            {
    ...
    if (string_starts_with(_instr, "RepeatEnd(")) 
    {
        ...
        { 
            show_debug_message("Error: RepeatEnd encountered without active loop on stack."); 
            StopInterpreter(); 
            return; 
        } 
        ...
        currentLoop.remaining_count -= loopNum; 
        loopNum++; 
    ...
    if (openParen > 0) 
    {
        ...
        { 
            show_debug_message(string_concat("Error: Mismatched parentheses: ", _instr));
            StopInterpreter();
            return;
        }
    } 
    ...
    if (commandName == "" || global.vars.CommandLibrary[$ commandName] == undefined) 
    { 
    ...
    { 
        ...
        { 
            show_debug_message(string_concat("Error: Variable '", varName, "' not found."));
            StopInterpreter();
            return;
        }
    }
