...
if (array_length(inventory.allItems) == 0) {
...
for (var i = 0; i < maxDisplay; i += 1) {
    ...
    if (index >= array_length(inventory.allItems)) break;

    var yPos = topLeft.y + 28 + (i * 32);
    var xPos = topLeft.x + 13;
    var slot = inventory.allItems[index];
...
var slot = inventory.allItems[selectedIndex];
